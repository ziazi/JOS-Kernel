...
trap_init(void)
{
  extern struct Segdesc gdt[];
    ...
    // we signatures of the handlers are made available for the comipler because the methods are defined in assembly trapentry.S 
    void th_divide();
    void th_debug();
    void th_nmi();
    void th_oflow();
    ...
    void th_irq_ide();
    void th_irq_error();
    // istrap=0 , cs= GD_KT, off= va of the handler, dpl =0 : must be in kernel mode/ring to invoke the handler
    //the order of the table of interrupts is followed in both this code and trapentry.S
    ...
    // for System calls istrap=0 by construction and we allow dpl to be 3 (user mode/ring)
