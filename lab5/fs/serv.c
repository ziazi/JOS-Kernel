...
serve_read(envid_t envid, union Fsipc *ipc)
{
  ...
  struct OpenFile *o;
  int hasFailed= openfile_lookup(envid, req->req_fileid, &o);
  if (hasFailed != 0) {
        return hasFailed;
    }
  ssize_t offset= file_read(o->o_file, ret->ret_buf, req->req_n, o->o_fd->fd_offset);
  if (offset > 0) {
    o->o_fd->fd_offset += offset;
  }
  return offset;
}
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
  ...
  struct OpenFile *o;
  uint32_t hasFailed= openfile_lookup(envid, req->req_fileid, &o);
  if (hasFailed != 0) {
        return hasFailed;
    }
  uint32_t offset= file_write(o->o_file, req->req_buf, req->req_n, o->o_fd->fd_offset);
  if (offset > 0) {
    o->o_fd->fd_offset += offset;
  }
  return offset;
